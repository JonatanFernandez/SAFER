...
#
#To Do: -Further error handling.
#       -Aditional filter for unique peptides.
#------------------------------------------------------
...
#     
SAFER <- function(x, Controls, Exps) {
  ...
  if (length(Controls) == 3 | length(Exps) == 3) {
    ...
    return(list(SelectedPreys = setNames(data.frame(rownames(SelectedPreys),SelectedPreys$FC),c("Selected Preys","|Fold Change")), 
